...
class Solution:
    ...
    @staticmethod
    def solve_all_paths_w_obstacles(maze: list[list[bool]], row: int, col: int, proc: str = ''):
        if row == len(maze) - 1 and col == len(maze[0]) - 1:
            local_list = []
            local_list.append(proc)
            return local_list
        ...
        if maze[row][col]:
            ...
            returned_list = []
            if row < len(maze) - 1 and col < len(maze[0]) - 1:
                returned_list.extend(Solution.solve_all_paths_w_obstacles(maze, row + 1, col + 1, proc + 'D'))
            if row < len(maze) - 1:
                returned_list.extend(Solution.solve_all_paths_w_obstacles(maze, row + 1, col, proc + 'VD'))
            if col < len(maze[0]) - 1:
                returned_list.extend(Solution.solve_all_paths_w_obstacles(maze, row, col + 1, proc + 'HR'))
            if row > 0: 
                returned_list.extend(Solution.solve_all_paths_w_obstacles(maze, row-1, col, proc + 'VU'))
            if col > 0:
                returned_list.extend(Solution.solve_all_paths_w_obstacles(maze, row, col - 1, proc + 'HL'))
            # Undoing what I did 
...
print(solution.solve_all_paths_w_obstacles(board, 0, 0))