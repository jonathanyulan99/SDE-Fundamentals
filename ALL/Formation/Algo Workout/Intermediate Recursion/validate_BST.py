...
A binary search tree is a binary tree where for each node, all values in the left subtree are smaller than the current node, 
...
def isValidBST(root: Node) -> bool:
    if root:
        def helper(root,_max=float('inf'),_min=float('-inf')):
            if not root:
                return True 
            ...
            return helper(root.left,root.value,_min) and helper(root.right,_max,root.value)
        return helper(root)
    return True 
