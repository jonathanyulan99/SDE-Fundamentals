...
def decodePhoneNumber(text: str) -> str:
    ...
    # iterate through each char in our input string
    for char in text:
        # make changes to ensure right casing, all inputs need to be converted to uppercase to pass into our mapping
        char = char.upper()
        # check to see if our char is an alphabetic string 
        # if it is then get the mapping and append it to our resulting string 
        if char.isalpha():
            result = result + letter_to_digit[char]
        # if it is not an alphabetic string 
        # then take the result and just append it back to our result
        # we are disregarding punctuation and numbers as that is the proper format 
        else:
            result += char 
